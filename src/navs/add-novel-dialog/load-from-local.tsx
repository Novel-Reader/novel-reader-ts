...
class LoadFromLocal extends Component {
  ...
  onUpload = () => {
    ...
    const tag = currentSelected ? currentSelected.map((option: any) => option.value).join(" ") : "";
    const fileObj = {
      name: filename.trim(),
      size: file.size,
      author,
  ...
  uploadToServer = (fileObj: any) => {
    const { name, author, context, abstract } = fileObj;
    const cover_photo = '';
    const price = 100;
    toaster.warning('正在上传中...');
    window.app.api.addNovel(name, cover_photo, author, context, price, abstract).then((res: any) => {
